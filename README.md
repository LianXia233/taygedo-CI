...
- **登录鉴权**：WebUI 与所有 API 需要账号密码登录（默认 `admin / admin`，**scrypt（N=16384, r=8, p=1）+ 常量时间比对**哈希，旧 `sha256` 单轮哈希在首次登录时自动升级为 scrypt），token 有效期 7 天，支持在线修改登录账号与密码，已签发的 token 会同步失效。
- **免鉴权模式（可选）**：设置环境变量 `TAYGEDO_NO_AUTH=1`（OpenWrt 下在 UCI 配置 `option no_auth '1'`）后，WebUI 与 LuCI 页面**无需登录**即可直接使用，适合内网自用场景。
- **应用层加密（0.5.0 新增）**：WebUI 与后端之间的请求/响应正文使用 **X25519 密钥交换 + HKDF-SHA256 + AES-256-GCM** 端到端加密，被动抓包无法还原正文（含明文口令与 Token）。详见[应用层加密](#应用层加密)。
...
访问 `http://<host>:8787`，默认账号密码 **`admin / admin`**（登录后请在「设置」中修改；若使用自动生成的随机初始口令，程序启动时会在控制台/日志打印一次，并在 WebUI 顶部提示修改）：
...
本方案不替代 HTTPS —— 它是 HTTPS 不可用时的等价兜底（自签名证书在浏览器端体验差，
自编译固件也不便申请公网证书），因此选择在应用层自带加密。
